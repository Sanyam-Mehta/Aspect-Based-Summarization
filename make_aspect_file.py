...
def get_binary_inputs(story_file):
    ...
    article_lines = ["%s %s %s" % (SENTENCE_START, line.split("\t")[1].strip(), SENTENCE_END) for line in
                     article_b.split("\n") if line != ""]
    ...
    with open(out_file, 'wb') as writer:
        for idx, s in enumerate(story_fnames):
            ...
            if os.path.isfile(os.path.join(in_dir, s)):
                ...
                raise Exception(
                    "Couldn't find tokenized story file %s in either tokenized story directories %s and %s. Was there an error during tokenization?" % (
    ...
    for idx, url in enumerate(url_list):
        ...
        if corpus in ['cnn', 'dailymail']:
            ...
            if aspect in TARGET_ASPECTS:
                try:
                    doc = open(os.path.join(dm_tokenized_stories_dir, url_hashes[idx] + ".story"), 'r').read().split(
                        "@highlight")[0]
                except:
                    doc = open(os.path.join(cnn_tokenized_stories_dir, url_hashes[idx] + ".story"), 'r').read().split(
                        "@highlight")[0]
...
def create_interleaved_docs(urls, hashes, aspect2hash, output_directory):
    ...
    while docs_created < len(hashes):
        ...
        while True:
            ...
            lengths = [DOC_2_LENGTHS[aspect2hash[this_aspect][this_tmp_idxs[idx]]] for idx, this_aspect in
                       enumerate(this_aspects)]

            if sum(lengths) < MAX_OUTPUT_DOC_LEN:
                break

        this_doc_hashes = [aspect2hash[aspect][this_tmp_idxs[idx]] + ".story" for idx, aspect in
                           enumerate(this_aspects)]
        ...
        for idx, hhash in enumerate(this_doc_hashes):
            with open(os.path.join(output_directory, hhash + "_" + str(docs_created) + ".story"), 'w+') as outfile:
                ...
                outfile.write("\n".join(
                    str(this_aspects[ii]) + "\t" + urls[hIdx] if ii != idx else "*" + str(this_aspects[ii]) + "\t" +
                                                                                urls[hIdx] for ii, hIdx in
                    enumerate(this_doc_idxs)) + '\n\n')
                outfile.write("\n".join(
                    str(this_aspects[ii]) + "\t" + hashes[hIdx] if ii != idx else "*" + str(this_aspects[ii]) + "\t" +
                                                                                  hashes[hIdx] for ii, hIdx in
                    enumerate(this_doc_idxs)) + '\n\n')
                outfile.write("\n".join(
                    str(this_sentence_aspects[sID]) + '\t' + sent for sID, sent in enumerate(this_output_doc)) + '\n\n')
...
if __name__ == '__main__':
    if len(sys.argv) != 6:
        print(
            "USAGE: python make_datafiles.py <url_list> <tokenized_cnn_stories_dir> <tokenized_dailymail_stories_dir> <out_txt> <out_binary>")
        sys.exit()
    ...
    if not os.path.exists(dm_tokenized_stories_dir): print(
        "Tokenizec Daily Mail corpus doesn't exist (at this location!)")
    ...
    for aspect in TARGET_ASPECTS_IDS:
        ...
        print("sum  ", aspect,
              len(train_aspects[TARGET_ASPECTS[aspect]]) + len(val_aspects[TARGET_ASPECTS[aspect]]) + len(
                  test_aspects[TARGET_ASPECTS[aspect]]))
    ...
    write_to_bin(all_test_urls, test_url_hashes, os.path.join(assembled_txt_dir, "test"), assembled_bin_dir,
                 os.path.join(assembled_bin_dir, "test.bin"), data_split="test")
    write_to_bin(all_val_urls, val_url_hashes, os.path.join(assembled_txt_dir, "val"), assembled_bin_dir,
                 os.path.join(assembled_bin_dir, "val.bin"), data_split="val")
    write_to_bin(all_train_urls, train_url_hashes, os.path.join(assembled_txt_dir, "train"), assembled_bin_dir,
                 os.path.join(assembled_bin_dir, "train.bin"), makevocab=True, data_split="train")
